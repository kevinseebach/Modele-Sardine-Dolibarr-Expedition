 ...
 */
class pdf_sardine extends ModelePdfExpedition
{
	...
	function __construct($db=0)
	{
		...
		$this->db = $db;
		$this->name = "Sardine";
		$this->description = "Modèle Sardine";
		...
		if ($conf->expedition->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $num; $i++)
				{
					...
					$libelleproduitservice = pdf_writelinedesc($pdf,$object,$i,$outputlangs,70,3,40,$curY,0,1);
					...
					$pdf->MultiCell(30, 3, $outputlangs->convToOutputCharset($object->lines[$i]->ref), 0, 'C', 0);


					$pdf->SetFont('','', $default_font_size - 3);
					//var_dump($object->lines[$i]);
					$pdf->SetXY(110, $curY);
					$weighttxt='';
					...
					$pdf->MultiCell(30, 3, $weighttxt, 0, 'C', 0);
					...
					if (! empty($conf->global->MAIN_PDF_DASH_BETWEEN_LINES) && $i < ($nblignes - 1))
					{
						...
						$pdf->line($this->marge_gauche-5, $nexY+1, $this->page_largeur - $this->marge_droite, $nexY+1);
				...
				$pdf->SetXY( 80,  $bottomlasttab);
				$pdf->MultiCell(30, 3, $outputlangs->transnoentities("Total"), 0, 'R', 0);


				if ($totalWeight!='') $totalWeighttoshow=showDimensionInBestUnit($totalWeight, 0, "weight", $outputlangs);
				$pdf->SetXY( 110,  $bottomlasttab);
				$pdf->MultiCell(30, 3, $totalWeighttoshow, 0, 'C', 0);


				$pdf->SetXY( 140,  $bottomlasttab);
				$pdf->MultiCell(30, 3, $totalOrdered, 0, 'C', 0);


				$pdf->SetXY( 170,  $bottomlasttab);
				$pdf->MultiCell(30, 3, $totalToShip, 0, 'C', 0);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->SetXY(5,$tab_top);
			$pdf->MultiCell(35,5,$outputlangs->transnoentities("Ref"),0,'C',1);
			$pdf->SetXY(40,$tab_top);
			$pdf->MultiCell(70,5,$outputlangs->transnoentities("Description"),0,'C',1);
			$pdf->SetXY(110,$tab_top);
			$pdf->MultiCell(30,5,$outputlangs->transnoentities("Weight"),0,'C',1);
		...
		$pdf->Rect(5, $tab_top, 195, $tab_height);
	}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($this->emetteur->logo)
		{
			if (is_readable($logo))
			{
			    ...
			    $pdf->Image($logo,10, 5, 150, $height);	// width=0 (auto)
			}
			else
			{
		...
		$Yoff = $Yoff+7;
		$pdf->SetXY($blSocX-80,$blSocY+11);
		$pdf->MultiCell(50, 8, "Date d'envoi : " . dol_print_date($object->date_creation,'day',false,$outputlangs,true), '', 'L');
		$pdf->SetXY($blSocX-80,$blSocY+15);
		$pdf->SetFont('','B', $default_font_size - 3);
		$pdf->SetTextColor(0,0,0);
		...
		$pdf->SetXY($blSocX-80,$blSocY+21);
		$pdf->SetFont('','B', $default_font_size );
		$pdf->SetTextColor(0,0,0);
		$pdf->MultiCell(40, 8, $outputlangs->transnoentities("Nombre de colis")." :  ", '', 'L');
		$pdf->SetDrawColor(192,192,192);
		$pdf->Rect(42, $blSocY+21, 10, 5);
